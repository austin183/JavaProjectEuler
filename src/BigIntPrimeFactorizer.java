...
class BigIntPrimeFactorizer {
    ...
    public void MakePrimesList(BigInteger upTo, String primeRecord)
    {
    ...
    public BigInteger GetSumPrimesUnderMaxValue(int maxValue) {
        BigInteger toReturn = BigInteger.ZERO;

        PrimeFactorizer primeFactorizer = new PrimeFactorizer();
        for(int i = 2; i < maxValue; i++)
        {
            if(primeFactorizer.IsPrime(i))
            {
                toReturn = toReturn.add(new BigInteger(Integer.toString(i)));
            }
        }
